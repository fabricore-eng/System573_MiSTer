# =============================================================================
# sys_pins.tcl -- MiSTer DE10-Nano board pin assignments for the 573 core.
#
# Our Konami_System_573.qsf pulls the MiSTer framework HDL via psx/sys/sys.qip
# (self-contained, qip-relative) instead of sourcing psx/sys/sys.tcl, because
# sys.tcl's relative file-refs (QIP_FILE sys/sys.qip etc.) resolve wrong from
# our project root and would re-pull the incompatible repo sys/. But sys.qip
# carries NO pin locations, so without this file every board pin (SDRAM, HDMI,
# VGA, ...) is auto-placed to an arbitrary ball -> the bitstream is mis-pinned
# (SDRAM dead -> BIOS can't run; VGA wrong -> CRT can't lock).
#
# These are the PIN assignments only (location + IO-standard/current-strength),
# extracted verbatim from the PSX framework's pin files. They are path-
# independent. The two files match upstream psx/PSX.qsf's
#   source sys/sys.tcl ; source sys/sys_analog.tcl
#
# Regenerate after a psx/ submodule bump:
#   grep -hE '^[[:space:]]*set_(location|instance)_assignment' \
#     psx/sys/sys.tcl psx/sys/sys_analog.tcl >> sys_pins.tcl
# =============================================================================

# ---- from psx/sys/sys.tcl (SDRAM, ADC, USER_IO, SPDIF, ... ) ----
...
# ---- from psx/sys/sys_analog.tcl (VGA_R/G/B/HS/VS, SDIO) ----
